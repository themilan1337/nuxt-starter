...
**Nuxt**: use the modules instead of manual setup:

```bash
pnpm i @pinia/nuxt @pinia/colada @pinia/colada-nuxt
pnpm i -D @pinia/colada-devtools
```
...
Global options go in `colada.options.ts` at the project root. Place `<PiniaColadaDevtools />` at the end of `app/app.vue` and import it explicitly from `@pinia/colada-devtools` (it is NOT auto-imported).
...
```ts
const selectedDeckId = ref<null | number>(null)
useQuery({
...
In Nuxt, the same folders live under `app/` (`app/api/`, `app/queries/`, `app/mutations/`) and fetch functions use `$fetch`.
